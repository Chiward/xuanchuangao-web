...
*   **意见反馈系统**：
    ...
    *   查看历史反馈记录与处理状态。
*   **范文参考（Examples）**：
    *   生成时可自动从 `examples/` 目录加载对应场景的最新范文（按文件修改时间取最新一份）。
    *   当前支持文本类范文：`.docx` / `.pdf` / `.txt`（图片范文暂不参与生成）。
...
*   **范文实时生效**:
    *   本地开发：直接替换 `examples/` 中的范文文件后，下一次生成会自动引用最新文件。
    *   部署环境：若以 Git/镜像方式部署到服务器，替换范文通常需要重新部署（除非部署环境支持在线更新服务器文件或将范文改为存储在数据库/对象存储中）。
